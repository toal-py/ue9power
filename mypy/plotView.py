...
def plotMonthlyShare(data):
    ...
    sns.barplot(data = {'red': sum(data.values())}, x = 100, hue = 'red', color='#d00000')
    sns.barplot(data = {'yellow': sum(data.values()) - data['red']}, x = 100, hue = 'yellow', color='#f48c06')
    sns.barplot(data = {'green': data['green']}, x = 100, hue = 'green', color='#74c69d')
    plotFile = BytesIO()    
    plotFigure.savefig(plotFile, format='png')
    plotFile.seek(0)
    encodedFile = base64.b64encode(plotFile.getbuffer())
