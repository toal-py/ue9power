...
def plotMonthlyShare(data):
    ...
    sns.barplot(data = dframe, x = 100, hue = 'red', color = sns.color_palette("Set2", 10)[0]) 
    sns.barplot(data = dframe, x = 100, hue = 'yellow', color=sns.color_palette("Set2", 10)[1])
    sns.barplot(data = dframe, x = 100, hue = 'green', color=sns.color_palette("Set2", 10)[2])      
    plotFile = BytesIO()    
    plotFigure.savefig(plotFile, format='png')
    plotFile.seek(0)
    encodedFile = base64.b64encode(plotFile.getbuffer())
